...
if (!empty($_POST['input_orderitem_validate'])) {
    ...
    if (mysqli_num_rows($select) > 0) {
        $message = '<script>alert("Order Yang di masukkan sudah ada ");
          window.location="../?x=orderitem&order=' . $kode_order . '&meja=' . $meja . '&pelanggan= ' . $pelanggan . '";
        </script>';
    } else {
        $query = mysqli_query($conn, "INSERT INTO tb_list_order (menu,kode_order,jumlah,catatan) values
         ('$menu','$kode_order','$jumlah','$catatan')");
        if ($query) {
